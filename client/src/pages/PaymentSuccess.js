...
const PaymentSuccess = () => {
  ...
  const fetchPaymentDetails = async () => {
    try {
      const response = await axios.post(
        "http://localhost:4000/api/paymentdetails/",
        {
      ...
      if (response) {
        ...
        // console.log(paymentDetails?.customer.name);
        downloadInvoice();
      } else {
        console.error("Error fetching payment details");
      }
  ...
  function downloadInvoice() {
    if (paymentDetails) {
      console.log(paymentDetails?.payment.products);
      var data = {
        ...
        // Your recipient
        "client": {
            "company": paymentDetails?.customer.name,
